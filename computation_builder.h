...
class ComputationBuilder {
 public:
  ...
  explicit ComputationBuilder(const string& computation_name);
  ...
  // The product of the new_size dimensions 
  // must equal the product of the operand's dimension sizes.
  //template <typename NativeT>
  ...
  std::unique_ptr<std::vector<NativeT>> Collapse(
     const std::vector<NativeT>& input_array,
     tensorflow::gtl::ArraySlice<int64> sizes,
     tensorflow::gtl::ArraySlice<int64> dimensions
  ) const
  {
     assert(sizes.size() == dimensions.size());

     std::unique_ptr<std::vector<NativeT>> result_ptr(new std::vector<NativeT>(input_array));

     std::vector<int> id_input(sizes.size());
     std::vector<int> sz_output(sizes.size());

     id_input[sizes.size() - 1] = 1;

     for (int64 n = sizes.size() - 1; n > 0 ; n--)
     {
        id_input[n-1] = id_input[n] * sizes[n];
     }

     std::vector<int> cntrs(sizes.size(), 0);
     int64 index = 0;
     ...
     if (dimensions.size() == 3)
     {
        ...
        std::string str = array3d->ToString();
...
)
{
   ...
   {
      dims[i] = tensor.size(dimensions[i]);
      dimsz = dimsz * dims[i];
   }

   std::vector<int64> sz(4, 0);

   int64 newsz = 1;

   for (size_t n = 0; n < sz.size(); n++)
   {
      sz[n] = new_sizes[n];
      newsz = newsz * new_sizes[n];
   }

   assert(dimsz = newsz);

   //std::unique_ptr <std::vector<NativeT>> output_vector = new std::vector<NativeT>(dimsz);

   std::vector<NativeT> output_array(newsz, (NativeT)0);
   ...
   for (int64 i0 = 0; i0 < dims[0]; i0++)
   {
      for (int64 i1 = 0; i1 < dims[1]; i1++)
      {
         for (int64 i2 = 0; i2 < dims[2]; i2++)
         {
            for (int64 i3 = 0; i3 < dims[3]; i3++)
            {
               //data_[dims_[3] * (dims_[2] * (dims_[1] * i + j) + k) + l];

               int64 input_id = i0 * (dims[1] * dims[2] * dims[3]) +
                  i1 * (dims[2] * dims[3]) + i2 * (dims[3]) + i3;

               int64 output_id = out0 * (new_sizes[1] * new_sizes[2] * new_sizes[3]) +
                  out1 * (new_sizes[2] * new_sizes[3]) + out2 * (new_sizes[3]) + out3;

               output_array[output_id] = input_data[input_id];

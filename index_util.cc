...
/* static */ int64 IndexUtil::MultidimensionalIndexToLinearIndex(
  ...
  for (auto dimension : shape.layout().minor_to_major()) {
    if (first) {
      ...
      scale = shape.dimensions(static_cast<int>(dimension));
      first = false;
    } 
    else 
    {
...
/* static */ std::vector<int64> IndexUtil::LinearIndexToMultidimensionalIndex(
  ...
  for (auto dimension : shape.layout().minor_to_major())
  {
    multi_index[dimension] =
        (linear_index / divisor) % shape.dimensions(static_cast<int>(dimension));
    divisor *= shape.dimensions(static_cast<int>(dimension));
  }
  return multi_index;
}

/* static */
bool IndexUtil::BumpIndices(const Shape& shape, std::vector<int64>* indices)
{
  for (int64 dimno = indices->size() - 1; dimno >= 0; --dimno)
  {
    int64 limit = shape.dimensions(static_cast<int>(dimno));
...
/* static */ int64 IndexUtil::GetDimensionStride(const Shape& shape,
  ...
  for (auto dim : layout.minor_to_major())
  {
    ...
    if (pdim_size == 0)
    {
      stride *= shape.dimensions(static_cast<int>(dim));
    }
    else
    {
      stride *= layout.padded_dimensions(static_cast<int>(dim));
    }
  }
  return stride;
}

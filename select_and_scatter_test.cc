...
void SelectAndScatterTest::R1S0F32() 
{
  ...
  ComputeAndCompareR1<float>(&builder_, {}, {}, ErrorSpec(1e-7f));
}

// Test for F32 1D array, when windows do not overlap.
void SelectAndScatterTest::R1F32() 
{
  const auto operand =
...
void SelectAndScatterTest::R1S32() 
{
...
void SelectAndScatterTest::R1S32OverlappingWindow() 
{
...
void SelectAndScatterTest::R2S32() 
{
  const auto operand =
...
void SelectAndScatterTest::ReshapeR2S32() 
{
...
void SelectAndScatterTest::R2S32OverlappingWindow() 
{
  const auto operand =
...
void SelectAndScatterTest::R2S32SamePadding() 
{
  const auto operand =
      builder_.ConstantR2<int32>({{7, 2, 5, 3, 8}, {3, 8, 9, 3, 4}});
  const auto source = builder_.ConstantR2<int32>({{2, 6, 4}});
...
// with each other.
void SelectAndScatterTest::R2S32SamePaddingOverlappingWindow() 
{
  const auto operand =
      builder_.ConstantR2<int32>({{7, 2, 5, 3, 8}, {3, 8, 9, 3, 4}});
  const auto source =
...
void SelectAndScatterTest::R2F32OverlappingR2Source() 
{
  ...
  builder_.SelectAndScatter(operand, ge_f32_, /*window_dimensions=*/{2, 2},
                            /*window_strides=*/{1, 1}, Padding::kValid, source,
                            builder_.ConstantR0<float>(0.0f), add_f32_);
  ComputeAndCompareR2<float>(&builder_, expected, {}, ErrorSpec(1e-7f));
}

void SelectAndScatterTest::R4F32Valid() 
{
  ...
  builder_.SelectAndScatter(operand, ge_f32_, {2, 3, 1, 1}, {2, 3, 1, 1},
                            ...
                            builder_.ConstantR0<float>(0.0f), add_f32_);
  ComputeAndCompareR4<float>(&builder_, e, {}, ErrorSpec(1e-7f));
}

void SelectAndScatterTest::R4F32Overlap() 
{
...
void SelectAndScatterTest::R4F32OverlapSmall() 
{
  Array2D<float> pzo = {{7.0f, 2.0f, 5.0f, 3.0f, 8.0f},
                        {3.0f, 8.0f, 9.0f, 3.0f, 4.0f},
                        {1.0f, 5.0f, 7.0f, 5.0f, 6.0f},
...
void SelectAndScatterTest::R4F32RefValidFixedSmall() 
{
...
void SelectAndScatterTest::R4F32RefSameRandom() 
{
...
void SelectAndScatterTest::R4F32RefSameRandomFullyPadded() 
{
...
void SelectAndScatterTest::R4F32RefValidRandom() 
{
...
void SelectAndScatterTest::R4F32RefValidRandomSmall() 
{
...
void SelectAndScatterTest::R1F32OverlappingWindowMaxScatter() 
{
...
void SelectAndScatterTest::R1F32OverlappingWindowMinScatter() 
{
  const auto operand = builder_.ConstantR1<float>({1, 2, 3, 100, 3, 2, 1});
  const auto source = builder_.ConstantR1<float>({34, 42, 53, 19});

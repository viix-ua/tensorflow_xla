...
/* static */ Shape ShapeUtil::NormalizeShapeToMonotonicDim0MajorLayout(
  ...
  {
    dims[i] = shape.dimensions(
       static_cast<int>(LayoutUtil::Major(shape.layout(), i))
    );
  }
  return MakeShapeWithMonotonicDim0MajorLayout(shape.element_type(), dims);
}
/* static */ 
void ShapeUtil::PopulateShape(
    PrimitiveType element_type, tensorflow::gtl::ArraySlice<int64> dimensions,
    Shape* shape) {
  shape->Clear();
...
/* static */ int ShapeUtil::TupleElementCount(const Shape& shape) {
  CHECK(IsTuple(shape));
...
/* static */ bool ShapeUtil::ShapeIs(const Shape& shape,
  ...
  for (int64 dimension : dimensions)
  {
    if (shape.dimensions(static_cast<int>(i)) != dimension)
    {
      return false;
    }
    i += 1;
...
/* static */ string ShapeUtil::HumanString(const ProgramShape& program_shape) {
  ...
  for (auto& shape : program_shape.parameters()) {
    const int i = static_cast<int>(parameters.size());
...
/* static */ bool ShapeUtil::SameDimensions(const Shape& lhs, const Shape& rhs)
{
...
/* static */
int ShapeUtil::GetDimension(const Shape& shape, int dimension_number)
{
  return static_cast<int>(
     shape.dimensions(GetDimensionNumber(shape, dimension_number))
     );
}

/* static */
int ShapeUtil::GetDimensionNumber(const Shape& shape, int dimension_number)
{
...
/* static */ Status ShapeUtil::ValidateShapeWithOptionalLayoutInternal(
  ...
  if (shape.element_type() == PRIMITIVE_TYPE_INVALID) {
    return InvalidArgument("shape has invalid element type");
  }
                                                 ...
                                                 const ShapeIndex& index)
{
  ...
  for (auto i : index) {
    ...
    return_shape = &return_shape->tuple_shapes(static_cast<int>(i));
  }
  return *return_shape;
}
                                                  ...
                                                  const ShapeIndex& index)
{
  ...
  for (auto i : index) {
    CHECK(IsTuple(*return_shape));
    return_shape = return_shape->mutable_tuple_shapes(static_cast<int>(i));
  }
  return return_shape;
}
  ...
  if (ShapeUtil::IsTuple(*shape)) {
    for (int64 i = 0; i < ShapeUtil::TupleElementCount(*shape); ++i) {
      ...
      TF_RETURN_IF_ERROR(ForEachMutableSubshapeHelper(
          shape->mutable_tuple_shapes(static_cast<int>(i)), func, index));
      index->pop_back();
    }
  }
  return Status::OK();
...
ShapeUtil::InsertedOrDeleted1SizedDimensions(const Shape& shape_pre,
  ...
  auto check_modified_dims = [&shape_pre, &shape_post, &deleted_indices,
    ...
    for (int64 modified_input_dim = prior_unmodified_dim_pair.first + 1;
      ...
      if (shape_pre.dimensions(static_cast<int>(modified_input_dim)) > 1) {
        return false;
      }
    ...
    for (int64 modified_output_dim = prior_unmodified_dim_pair.second + 1;
         ...
         ++modified_output_dim) {
      if (shape_post.dimensions(static_cast<int>(modified_output_dim)) > 1) {
        return false;
      }
...
/* static */ bool ShapeUtil::ReshapeIsBitcast(const Shape& input_shape,
  ...
  auto check_input_unit_indices = [](const Shape& input_shape,
    ...
    for (int input_dim = 0; input_dim < ShapeUtil::Rank(input_shape); ++input_dim)
    {
...
/* static */ Shape ShapeUtil::DeleteDimension(int64 dim_to_delete,
  ...
  if (LayoutUtil::HasLayout(shape)) {
    ...
    for (int i = 0; i < layout->minor_to_major().size();) {

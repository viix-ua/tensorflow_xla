...
class ShapeUtil {
 public:
  ...
  static int Rank(const Shape& shape) { return shape.dimensions_size(); }
  ...
  static int GetDimension(const Shape& shape, int dimension_number);
  ...
  static int GetDimensionNumber(const Shape& shape, int dimension_number);
  ...
  static int TupleElementCount(const Shape& shape);

  // Returns the tuple element shape at given index.
  // Precondition: IsTuple(shape) && TupleElementCount(shape) > index
  static const Shape& GetTupleElementShape(const Shape& shape, int index);
  ...
  static Status ForEachMutableSubshape(Shape* shape, MutatingVisitorFunction func);

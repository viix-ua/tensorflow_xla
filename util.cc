...
void LogLines(int sev, tensorflow::StringPiece text, const char* fname, int lineno)
{
...
int64 Product(tensorflow::gtl::ArraySlice<int64> xs) {
  return std::accumulate(xs.begin(), xs.end(), int64(1), std::multiplies<int64>());
}
    ...
    tensorflow::gtl::ArraySlice<int64> b)
{
       ...
       ;)
  {
    ...
    bool in_bounds_i = i < int64(a.size());
    bool in_bounds_j = j < int64(b.size());
